    ...
    <div className="bg-base-100 rounded-xl border border-base-300 shadow-sm overflow-hidden">
      ...
      >
        <div className="flex-1 flex flex-wrap items-center gap-2 min-w-0">
          ...
          <span className="text-sm text-base-content truncate max-w-[200px]">{order.item_name || order.sku || '—'}</span>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0">
          <span className="text-xs text-base-content/40 hidden sm:inline">{formatDate(order.ordered_at)}</span>
          <span className="text-xs text-base-content/50 hidden sm:inline">×{order.quantity || 1}</span>
          <span className="font-bold text-sm text-base-content">£{(Number(order.unit_price || 0) * Number(order.quantity || 1)).toFixed(2)}</span>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-base-200 flex flex-col gap-4 pt-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <p className="text-xs text-base-content/40 mb-0.5">Order Ref</p>
              <span className="font-mono text-xs">{order.order_number || order.platform_order_id?.slice(0, 16) || '—'}</span>
            </div>
            <div>
              <p className="text-xs text-base-content/40 mb-0.5">SKU</p>
              <span className="font-mono text-xs">{order.sku || '—'}</span>
            </div>
          </div>
...
export const Orders: React.FC<OrdersProps> = ({ orders, onRefresh }) => {
  ...
  const filtered = orders.filter(o => {
    ...
    if (search) {
      ...
      if (!o.sku?.toLowerCase().includes(q) &&
